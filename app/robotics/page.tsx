...
// 「日本の必然 → いま起きている変化 → まだ、誰もできていない → 産業化後の世界」の物語順
const whyNowReasons = [
  {
    tag: "日本の必然",
    value: "12倍",
      ...
      <section className="qrd-sec" id="why-now" aria-label="なぜ今、水中ロボティクスなのか">
        <div className="rv">
          ...
          <p className="qrd-lead">
            内閣府は「AUVの社会実装に向けた戦略」を掲げ、官民プラットフォームで産業育成を進めています。産業化の先にあるのは、人が危険な海に出ずに、制御室から水中ロボットが海を守る世界。それがなぜ日本の必然なのか、数字で見てください。
          </p>
        </div>
        ...
        <p className="qrd-src rv">
          出典: 内閣府「自律型無人探査機（AUV）の社会実装に向けた戦略」、AUV官民プラットフォーム「AUV等海洋ロボティクス導入の効果」（2026年2月）、内閣府海洋政策資料（管轄海域は世界第6位）、海上保安庁（領海・排他的経済水域 約447万km²）、総務省（国際通信に占める海底ケーブルの割合）、経済産業省・国土交通省「洋上風力産業ビジョン」、日本財団-GEBCO「Seabed 2030」（世界の海底地形の地図化率 約29%・2026年時点）、チャレンジャー海淵の水圧は約108MPa（≒約1,100気圧・1cm²あたり約1.1トン）、JAMSTEC「うらしま8000」（8,015m・国産巡航型AUVの日本記録）、ハルビン工程大学「悟空号」（10,896m・AUV世界最深記録）。削減率等は一定の条件設定に基づく政府試算の目安です。
        </p>
      </section>
